...
# Pull docker container before running script (Or make sure blastn is in PATH) 
# LSF_DOCKER_PRESERVE_ENVIRONMENT=false bsub -q general-interactive -Is -a 'docker(ncbi/blast:latest)' /bin/bash
# latest blast version Nov2020: version=2.11.0
# Make Blast DB
# makeblastdb -in KM360090.1.fasta -dbtype nucl -parse_seqids
...
\t \033[1m-v  VODKA filtered results table\033[0m (*_RESULTS.filtered.txt)
...
./VODKA2_results_to_fasta_v2.0b.sh $(basename ${SAMPLETXT}) || exit 1
...
join -j1 -o "0,1.7" ${sample}.blast.sorted ${sample}.blast.junctions.2 > ${sample}.blast.junctions.2.strands
    ...
    if [[ $(( $i % 2 )) == 0 ]]; then
      ...
      if (( ${range1end} < ${range2start} )); then
        ...
        if (( ${BREAKminusN} <= ${range1end} && ${range1end} <= ${BREAKplusN} && ${REJOINminusN} <= ${range2start} && ${range2start} <= ${REJOINplusN} )) ; then
          ...
          echo ${li} | cut -d " " -f 1
          # print to file blast B R SIZE
          # delVG size is (gl - Break + Rejoin +1)
          DVGsize=$(( ${gl} - ${REJOIN} + ${BREAK} + 1 ))
          delsize=$(( ${REJOIN} - ${BREAK} - 1 ))
          echo ${BREAK} ${REJOIN} ${DVGsize} ${delsize} >> ${sample}.blast.list
        fi
    ...
    if [[ $(( $i % 2 )) == 0 ]]; then
      ...
      if (( ${range1start} < ${range2start} )); then
        ...
        if (( ${BREAKminusN} <= ${range1start} && ${range1start} <= ${BREAKplusN} && ${REJOINminusN} <= ${range2start} && ${range2start} <= ${REJOINplusN} )) ; then
          ...
          # print to file blast B R SIZE
          # delVG size is (gl - Break + Rejoin +1)
          DVGsize=$(( ${gl} - ${BREAK} + 1 + ${gl} - ${REJOIN} + 1 ))
          loopsize=$(( ${REJOIN} - ${BREAK} ))
          stemsize=$(( $(( ${gl} - ${REJOIN} + 1 )) * 2 ))
          perstem=$(awk "BEGIN{print ${stemsize}/${DVGsize}*100}" )
          echo ${BREAK} ${REJOIN} ${DVGsize} ${loopsize} ${stemsize} ${perstem} >> ${sample}.blast.list
        fi
...
# double check blast list
# get species occurence/size/delsize (species is defined by B R not SIZE) and sort by B and R
sort ${sample}.blast.list | uniq -c | sed "s/^ *//g" | sort -k2n -k3n > ${sample}.blast.list.sorted
...
# o1/O: junction occurence, b1/B/r1/R: BREAK and REJOIN positions, s1/S: DVG size, d1/D: extra info about DVG
o1=0; b1=0; r1=0; s1=0; d1=0; flag=0
while read -r O B R S D ; do
  if [[ ${flag} == 0 ]]; then
    # prev species is new
    # if prev_b -5 <= current_B <= prev_b +5 AND prev_r -5 <= current_R <= prev_r +5
    if (( $(( ${b1} - ${N} )) <= ${B} && ${B} <= $(( ${b1} + ${N} )) && $(( ${r1} - ${N} )) <= ${R} && ${R} <= $(( ${r1} + ${N} )) )); then
      # store new B and R and sum of occurences
      b1=${B}; r1=${R}; s1=${S}; d1=${D}; o1=$(( ${o1} + ${O} ))
      flag=1
    else
      # print to file prev species info
      echo ${o1} ${b1} ${r1} ${s1} ${d1}
      # store current species info
      b1=${B}; r1=${R}; s1=${S}; d1=${D}; o1=${O}
    fi
  # this species already exists
  # if prev_b -5 <= current_B <= prev_b +5 AND prev_r -5 <= current_R <= prev_r +5
  elif (( $(( ${b1} - ${N} )) <= ${B} && ${B} <= $(( ${b1} + ${N} )) && $(( ${r1} - ${N} )) <= ${R} && ${R} <= $(( ${r1} + ${N} )) )); then
    # store new B and R and sum of occurences
    b1=${B}; r1=${R}; s1=${S}; d1=${D};  o1=$(( ${o1} + ${O} ))
  else
    # print to file prev species info
    echo ${o1} ${b1} ${r1} ${s1} ${d1}
    # store current species info
    b1=${B}; r1=${R}; s=${S}; d1=${D};  o1=${O}
    flag=0
  fi
done < ${sample}.blast.list.sorted > ${sample}.2ranges.confirm.blast-species.txt

# Don't forget last row of file (not processed at the end of loop)
IFS=' ' read -r O B R S D <<< $(tail -n 1 ${sample}.blast.list.sorted)
if [[ ${flag} == 1 ]]; then
  # if this species is equivalent to previous one => sum occurences
  O=$(( ${o1} + ${O} )) 
fi
# print to file
echo ${O} ${B} ${R} ${S} ${D} >> ${sample}.2ranges.confirm.blast-species.txt

# Add header to file
if [ ${DVGTYPE} = "DEL" ]; then header="NB BREAK REJOIN SIZE delSIZE";
elif [ ${DVGTYPE} = "CB" ]; then header="NB BREAK REJOIN SIZE loopSIZE stemSIZE stemPERC";
fi
sed -i "1s/^0/${header}\n/" ${sample}.2ranges.confirm.blast-species.txt
sed -i "/^ 0/d" ${sample}.2ranges.confirm.blast-species.txt
...
blastjunc=$(( $(wc -l ${sample}.2ranges.confirm.blast-species.txt | cut -d " " -f 1) - 1 ))
reads=$(( $(wc -l ${sample}.2ranges.confirm.vodka.txt | cut -d " "  -f 1) - 1 ))
echo " * Found ${vodkajunc} ${DVGTYPE} DVG \"junctions\" and ${blastjunc} \"BLAST species\" (supported by ${reads} DVG reads) *"
